...
def bboxes_nms(classes, scores, bboxes, nms_threshold=0.45, sigma=0.5, Nt=0.7, method=0, thresh=0.2, appr_factor=10.0,
                attributes=None):
    boxes_corners = boxes_3d_to_corners(bboxes)
    boxes_corners = np.int32(boxes_corners * appr_factor)
    dets = np.concatenate((boxes_corners.reshape((boxes_corners.shape[0], -1)), scores[:, None]), axis=1)
    box_len = len(dets)

    for i in range(box_len):
        max_pos = i

        for pos in range(i + 1, box_len):
            if dets[max_pos, -1] < dets[pos, -1]:
                max_pos = pos

        dets[[i, max_pos]] = dets[[max_pos, i]]
        if attributes is not None:
            attributes[[i, max_pos]] = attributes[[max_pos, i]]

        for pos in range(i + 1, box_len):
            # Dummy IoU calculation for 3D boxes - replace with actual logic
            iou = 0.5  # Placeholder for actual IoU calculation
            if method == 1:  # Linear
            ...
            dets[pos, -1] *= weight
            if dets[pos, -1] < thresh:
                dets[pos, :-1], dets[box_len - 1, :-1] = dets[box_len - 1, :-1], dets[pos, :-1]
                dets[pos, -1], dets[box_len - 1, -1] = dets[box_len - 1, -1], dets[pos, -1]
                box_len -= 1
                pos -= 1

    keep = np.arange(box_len)
    kept_classes = classes[keep]
    kept_scores = scores[keep]
    kept_bboxes = bboxes[keep]
    kept_attributes = attributes[keep] if attributes is not None else None

    return kept_classes, kept_scores, kept_bboxes, kept_attributes
