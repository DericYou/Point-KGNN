...
class RandomRotateZ(object):
    ...
    def __call__(self, data):
        ...
        data = np.dot(data, R)  # anti clockwise
        return data
...
class RandomScaleAndJitter(object):
    ...
    def __call__(self, data):
        ...
        for i, s in enumerate(self.scale_xyz):
            if not s: scale[i] = 1
        jitter = np.clip(self.noise_sigma * np.random.randn(data.shape[0], 3), -self.noise_clip, self.noise_clip)
        data = data * scale + jitter
        return data
...
class RandomShift(object):
    ...
    def __call__(self, data):
        shift = np.random.uniform(-self.shift[0], self.shift[0], 4)
        data += shift
        return data
...
class RandomScaleAndTranslate(object):
    ...
    def __call__(self, data):
        ...
        data = np.add(np.multiply(data, scale), shift)
...
class RandomJitter(object):
    ...
    def __call__(self, data):
        jitter = np.clip(self.noise_sigma * np.random.randn(data.shape[0], 4), -self.noise_clip, self.noise_clip)
        data += jitter
        return data
